...
//        }
    ...
    private void initPhotoGrid() {
        ...
        ((DefaultItemAnimator) photoList.getItemAnimator()).setSupportsChangeAnimations(true);
    }
    ...
    private void initAdapter() {
        adapter = new PhotoListAdapter(viewModel.getPhotoModels());
        adapter.setOnRetryListener(position -> {
            PhotoModel photoModel = viewModel.getPhotoModels().get(position);
            photoModel.retry();
        });

        adapter.setOnEndList(() -> viewModel.startRetrieveCookie("andy", "1234"));
            ...
            public void onPropertyChanged(Observable observable, int id) {
                ...
                if (id == BR.photoModels) {
                    if (viewModel.getPhotoModels() != null && adapter != null) {
                        if (!binding.photoList.isScrollContainer()) {
